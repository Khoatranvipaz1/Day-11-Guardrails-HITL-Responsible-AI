...
def content_filter(response: str) -> dict:
    ...
    pii_patterns = {
        ...
        "API key": r"\bsk-[a-zA-Z0-9_-]{8,}\b",
        "password": r"\bpassword\s*(?::|=|\bis\b)\s*['\"]?[^\s,'\";]+",
        "internal host": r"\b[a-zA-Z0-9.-]+\.internal(?::\d{2,5})?\b",
    ...
    async def after_model_callback(
        ...
        *,
        ...
        if self.use_llm_judge:
            judgment = await llm_safety_check(checked_text)
            if not judgment["safe"]:
                llm_response.content = types.Content(
                    role="model",
                    parts=[types.Part.from_text(
                        text="I cannot provide that response. Please ask a banking support question."
                    )],
                )
